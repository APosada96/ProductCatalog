...
public sealed class UpdateProductHandlerTests
{
    ...
    [Fact]
    public async Task When_Success_Should_Update_Save_Publish_ProductChanged_And_Invalidate_Caches()
    {
        // Arrange
        var writeRepo = new Mock<IProductWriteRepository>(MockBehavior.Strict);
        var readRepo = new Mock<IProductReadRepository>(MockBehavior.Strict);
        ...
        // limpiamos eventos del ctor para contar solo cambios
        product.ClearEvents();
        ...
        // Assert persist
        writeRepo.Verify(x => x.UpdateAsync(It.Is<Product>(p => p.Id == product.Id), It.IsAny<CancellationToken>()), Times.Once);
        ...
        mediator.Verify(x => x.Publish(
            It.Is<CacheInvalidationRequested>(n => n.CacheKey == CacheKeys.ProductById(product.Id)),
        ...
        // Assert domain changed published at least once
        mediator.Verify(x => x.Publish(
            It.IsAny<DomainEventNotification<ProductChanged>>(),
            It.IsAny<CancellationToken>()),
            Times.AtLeastOnce);

        mediator.Verify(x => x.Publish(It.IsAny<INotification>(), It.IsAny<CancellationToken>()), Times.AtLeast(3));
